...
*/
    ...
    public Set<Quiz> getQuizAchetesParEtablissement(int idEtablissement) throws SQLException {
        Set<Quiz> quizzes = new HashSet<>();

        String req = "SELECT q.* FROM quiz q " +
                "JOIN etablissement_quiz eq ON q.id_quiz = eq.id_quiz " +
                "WHERE eq.id_etablissement = ?";
        try (PreparedStatement pst = cnx.prepareStatement(req)) {
            pst.setInt(1, idEtablissement);
            ResultSet rs = pst.executeQuery();

            while (rs.next()) {
                Quiz quiz = new Quiz();
                quiz.setId_quiz(rs.getInt("id_quiz"));
                quiz.setCode_quiz(rs.getInt("code_quiz"));
                quiz.setNom_quiz(rs.getString("nom_quiz"));
                quiz.setDescription(rs.getString("description"));
                quiz.setPrix_quiz(rs.getInt("prix_quiz"));
                quizzes.add(quiz);
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
